...
App `src/stores/` 与 admin-core 共用 `createAdminPinia()` 上已安装的 persistedstate 插件，不要再 `createPinia`。未写 `persist` 的 store 不会落盘。Pinia id 使用 appCode 点分前缀（如 `{appCode}.foo`）。
...
- 表单录入控件都必须有 `placeholder`：输入「请输入…」，选择「请选择…」。`InSelect` 已声明 `placeholder` 并传给内部 `el-select`，调用处必须传入。列表搜索仍用「搜索…」，不要和表单占位混用。开关、复选框、单选和纯展示不要求占位

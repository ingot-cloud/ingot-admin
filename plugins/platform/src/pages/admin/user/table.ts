...
export const ADMIN_USER_TABLE_ID = "platform-admin-user";
...
export function createAdminUserToolbarActions(
  onCreate: () => void,
): Array<InTableAction<SysUser>> {
  return [
    {
      key: "create",
      label: "添加用户",
...
export function createAdminUserRowActions(
  row: SysUser,
  handlers: {
    onDetail: (row: SysUser) => void;
    onToggleEnabled: (row: SysUser) => void;
    onLock: (row: SysUser) => void;
    onResetPassword: (row: SysUser) => void;
  },
): Array<InTableAction<SysUser>> {

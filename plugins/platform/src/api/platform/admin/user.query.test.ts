...
import { PlatformAdminUserPageQueryOptions, platformAdminUserQueryKeys } from "./user.query";
...
vi.mock("./user", () => ({
  ...
  UserOrgInfoAPI: vi.fn(),
}));

describe("PlatformAdminUserPageQueryOptions", () => {
  it("列表 Key 不含明文手机号且 queryFn 透传 condition", async () => {
    const { UserPageAPI } = await import("./user");
    expect(platformAdminUserQueryKeys.lists()).toEqual(["platform", "admin-user", "list"]);
    const options = PlatformAdminUserPageQueryOptions({
      current: 1,
      size: 20,

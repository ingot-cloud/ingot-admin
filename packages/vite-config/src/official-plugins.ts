 ...
 * - 同时排除 `@ingot/admin-core` / `@ingot/admin-common`，避免预构建改写 `import.meta.glob` 路径键（布局/系统页扫不到）
 *
  ...
  return {
    ...
    config(): UserConfig {
      return {
        ...
        optimizeDeps: {
          exclude: [...plugins.map((plugin) => plugin.packageName), "@ingot/admin-core", "@ingot/admin-common"],
        },
        server: {
          fs: {
            allow: [

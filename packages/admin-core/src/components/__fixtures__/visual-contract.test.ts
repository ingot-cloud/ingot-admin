...
describe("admin UI visual fixtures", () => {
  it("主布局不再使用框架级 overflow-x-hidden", () => {
    ...
    expect(detailDrawer).toContain("alignContent");
    expect(detailDrawer).toContain("currentTabEditable");
    expect(detailDrawer).toContain("in-form-skeleton");
    expect(detailDrawer).not.toContain(":loading=\"loading\"");
    const formSkeleton = readFileSync(resolve(root, "../form/InFormSkeleton.vue"), "utf8");
    expect(formSkeleton).toContain("in-form-skeleton");
    expect(formSkeleton).toContain("gap: var(--in-space-2)");
    const bizTabsRoot = readFileSync(resolve(root, "../tabs/InBizTabs.vue"), "utf8");
    expect(bizTabsRoot).toContain("--in-biz-tabs-inline-padding");
    expect(bizTabsRoot).toContain("is-aligned");
    expect(bizTabs).toContain("in-biz-tabs-header__bar");
    expect(bizTabs).toContain("var(--in-biz-tabs-inline-padding");
    expect(bizTabs).toContain("padding: 12px 0 10px");
    const inForm = readFileSync(resolve(root, "../form/InForm.vue"), "utf8");
    expect(inForm).toContain("in-detail-form");
    expect(inForm).toContain("is-editing");
    expect(inForm).toContain("var(--in-text-color-placeholder)");
    expect(inForm).toContain("color: var(--in-text-color)");
    expect(inForm).toContain("--in-detail-field-editor-display");
    expect(inForm).toContain(".in-detail-field.has-editor");
    expect(inForm).toContain("gap: var(--in-space-2)");
    expect(inForm).toContain(":deep(.el-input)");
    const inSelect = readFileSync(resolve(root, "../select/InSelect.vue"), "utf8");
    expect(inSelect).toContain("in-select");
    expect(inSelect).toContain("--el-select-width: auto");
    expect(inSelect).toContain("min-width: calc(var(--in-space-8) * 5)");
    const detailField = readFileSync(resolve(root, "../form/InDetailField.vue"), "utf8");
    expect(detailField).toContain("overflow-x: auto");
    expect(detailField).toContain("border-radius: var(--in-radius-control)");
    expect(detailField).toContain("--in-detail-field-editor-display");
    const descriptionItem = readFileSync(resolve(root, "../description/InDescriptionItem.vue"), "utf8");
    expect(descriptionItem).toContain("var(--in-text-color-placeholder)");
    expect(descriptionItem).toContain("overflow-x: auto");
    expect(descriptionItem).toContain("gap: var(--in-space-2)");
  ...
  it("顶栏与侧栏尺寸 Token 已对齐验收值", () => {
    ...
    expect(tokens).toContain("--in-menu-arrow-size: 14px");
    expect(tokens).toContain("--in-menu-icon-gap: var(--in-space-2)");
    expect(tokens).toContain("--in-menu-base-level-padding: 16px");
    expect(tokens).toContain("--in-menu-nested-indent: 2em");
    expect(tokens).toContain("--in-menu-text-plain-color: var(--in-black)");
    expect(tokens).toContain("--in-menu-text-active-color: var(--in-gray-950)");
    expect(tokens).toContain("--in-gray-950: #1f2329");
  ...
  it("侧栏滚动视口与底部控制分离，无 collapse popper", () => {
    ...
    expect(menu).toContain("in-menu-node.is-root");
    expect(menu).toContain("transition: all 0.2s ease-in-out, color 0s, font-weight 0s");
    expect(menu).toContain("down-arrow.svg");
    expect(menu).toContain("--in-menu-arrow-size");
    expect(menu).toContain("is-root > .el-sub-menu__title .el-sub-menu__icon-arrow");
    expect(menu).toContain(".el-sub-menu.is-active > .el-sub-menu__title:hover");
    ...
    expect(submenu).toContain("is-root");
  ...
  it("顶栏按品牌/导航/搜索/小部件/用户五区划分", () => {
    ...
    expect(mainCss).toContain("font-family: var(--in-font-family)");

...
| `InAppBar` | 全局顶栏 | 品牌 / 大类菜单 / 搜索 / 功能小部件 / 用户入口五区；品牌宽与侧栏对齐；宽度不足时收纳导航、搜索变图标、小部件从右向左进入更多 | `header` 配置与旧 `utilities` prop、`#nav` / `#header-start` / `#header-end` / `#utilities` 仍可用；旧插槽作为不可拆整体收纳 |
...
| `InBizTabs` | 页内 Tab | 16px；默认 `--in-text-color`，选中 `--in-color-primary`；墨条跟文案同宽、顶部圆角、半条压线；`before-change` 可拦截 | `v-model` + `change`；键盘方向键；`InBizTabPanel` 默认 `lazy`，未激活不挂载，激活过一次后切走只隐藏 |
| `InDrawer` | 长任务编辑 | 中性标题、固定操作区、无装饰竖条；`layout="pinned"` 钉住内容头、仅内部滚动；遮罩默认透明；全屏向导用 `close-position="start"` 把关闭放到标题栏左侧 | `v-model`、`title`、`#header`/`#footer`、`loading`；默认 `layout="default"`；需要压暗时设 `overlay-color`（如 `var(--in-overlay-mask)`） |
...
| `InDialog` | 短确认/小表单 | `description`、`tone: default \| danger`；标题左侧 `#icon`；`showClose` 控制右上角关闭；`align-center` 全屏居中；内容区内边距与页头水平对齐；`layout="pinned"` 定高、头脚固定、只滚中间 | `v-model`、`title`、`#footer`、`#header` |
...
type InDialogLayout = "default" | "pinned";
type InDrawerLayout = "default" | "pinned";
type InDrawerClosePosition = "end" | "start";

...
| org      | `pages/contacts/user/IndexPage.vue`      | Split List | 去掉页内路由 Tab；姓名列 `InAvatar`；状态列 `InAccountStatusTag`；暂停/恢复账号进入更多菜单；人数在 title 右侧，不使用 `#summary` |
...
- `org/contacts/user` 姓名列使用 `InAvatar`：无头像时圆圈内显示姓名最后两字；状态列使用 `InAccountStatusTag`（正常 / 已暂停 / 已锁定）。行内只保留详情；暂停账号/恢复账号与删除进入更多菜单。更新接口只写 `enabled`。表格「共 N 人」放在 `#title` 右侧（`.in-table__count`），不使用 `#summary`。

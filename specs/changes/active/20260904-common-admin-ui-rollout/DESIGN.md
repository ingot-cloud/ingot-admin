...
| org      | `pages/contacts/user/IndexPage.vue`      | Split List | 去掉页内路由 Tab；姓名列 `InAvatar`；状态列 `InAccountStatusTag`；暂停/恢复账号进入更多菜单 |
...
- `org/contacts/user` 姓名列使用 `InAvatar`：无头像时圆圈内显示姓名最后两字；状态列使用 `InAccountStatusTag`（正常 / 已暂停 / 已锁定）。行内只保留详情；暂停账号/恢复账号与删除进入更多菜单。更新接口只写 `enabled`。

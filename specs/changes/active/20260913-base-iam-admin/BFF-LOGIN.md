...
`*.localhost` 直接回环，不必写 `/etc/hosts`。不要配 `Domain=.localhost`，也不要用光杆 `http://localhost:端口` 或 `http://IP:端口` 当作注册 origin。OAuth `oauth-redirect-uri` 仍是网关协议回调，浏览器不访问。端到端步骤见后端仓库 `docs/modules/authorization-server/BFF-AUTH-FLOW.md`。局域网互访应绑同样主机名（最好 HTTPS）。IP+HTTP 不是受支持的部署形态。

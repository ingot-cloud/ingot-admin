...
> 修订日期：2026-09-18。状态：implementing。用户已确认两套入口、不传重定向 URL、Host→appId 与 Nacos 回跳。Cookie 策略跟 `require-https`，注册表仅 Gateway/BFF 加载。主 change 既有 IAM 证据保留。本文是 BFF 登录、跳转与会话的权威契约；端到端调用链见后端 `docs/modules/authorization-server/BFF-AUTH-FLOW.md`。IAM 管理面仍以 API/OpenAPI 为准。
...
`.local` 不会自动回环，本机 `/etc/hosts` 需指向 `127.0.0.1`。不要配 `Domain=.local`，也不要用 `http://localhost:端口` 或 `http://IP:端口` 当作注册 origin。OAuth `oauth-redirect-uri` 仍是网关协议回调，浏览器不访问。端到端步骤见后端仓库 `docs/modules/authorization-server/BFF-AUTH-FLOW.md`。局域网互访应绑同样主机名（最好 HTTPS）。IP+HTTP 不是受支持的部署形态。

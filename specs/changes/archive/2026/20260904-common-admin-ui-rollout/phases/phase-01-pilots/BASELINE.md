...
| `org/contacts/user` | `/org/contacts/user` | Split List | `contained` + `workspace` | 左树 / 右表数据区 | `org-contacts-user` | 部门树 + 成员分页；点树节点后按 `deptId` 查询；账号状态筛选传 `enabled` | 添加成员、账号状态筛选、详情、启停、删除、字段设置、左栏折叠 |

...
- [x] [P] 使用至少 200 行 fixture 检查固定区域、独立纵横滚动、固定操作列和分页可达性。代表性列表的 200 行操作映射已由 `table.test.ts` 覆盖；真实滚动需登录后在浏览器核对。
- [x] [P] 检查“详情 + 至多一个高频动作 + …”、必选列与字段选择；组织成员页无批量组，不新增无接口按钮。
- [x] 汇总试点偏差并更新 change 文档；涉及已批准设计变化时等待用户重新确认。
- [x] 取得用户视觉验收后再进入 Phase 02。用户于 2026-09-07 要求以通讯录-成员管理为标准完成剩余迁移。

...
- 大类分组入口按配置以 hover 或 click 展开面板，缺省 hover；宽屏按组并排，组标题为灰字加底部分割线并横跨该组全部菜单列。组内菜单项默认按数量动态折列（每列最多 8 行，最多 4 列），也可在分组上写 `columns` 指定列数；条目为图标 + 文案、固定列宽、悬停浅底。窄屏强制单列。面板落在顶栏下方，不遮盖触发入口；最大高度受视口约束并内部滚动。
- 更多浮层点击分组入口后，在同一浮层中展示分组内容并提供返回按钮，避免多层浮层互相遮挡。
- 打开同一区域新面板时关闭旧面板；提供焦点进入、键盘选择、Escape、关闭后回到触发器。被响应式隐藏的触发器关闭浮层并把焦点交给仍可见的更多入口。
- 内置用户动作复用现有确认、修改密码对话框和退出逻辑；空菜单不渲染箭头或空下拉。自定义回调支持 Promise，执行期间避免重复触发，失败沿用项目消息能力展示并恢复可操作状态。
- 顶栏图标按钮等默认文字提示使用深色 Tooltip（白字、带箭头），与框架其它 `ElTooltip` 一致，不再使用 `effect="light"`。
